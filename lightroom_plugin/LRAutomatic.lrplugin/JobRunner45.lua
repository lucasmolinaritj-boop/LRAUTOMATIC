...
-- IMPORTANTE: nenhuma operação SDK que possa fazer yield fica dentro de pcall.
...
local function buildSmartPreviews(catalog, photos, job)
    ...
    job.smart_previews_created=#created; job.smart_previews_existed=#existed; job.smart_previews_failed=#failed
...
local function finishJob(jobPath, job, failed)
    refreshTotals(job)
    job.current_source=nil; job.finished_at=timestamp()
...
local function startStandardPreviews(photos, jobPath, job, baseFailed)
    ...
    job.standard_previews_status='running'; job.standard_previews_created=0; job.standard_previews_failed=0
    safeWriteJob(jobPath, job)
    local batch={remaining=#photos,handles={},finished=false}
    ...
    local function completeOne(success, errorMessage)
        ...
        if success then job.standard_previews_created=job.standard_previews_created+1 else job.standard_previews_failed=job.standard_previews_failed+1; plainLog('STANDARD_PREVIEW_FAILED ' .. tostring(errorMessage)) end
        ...
        if active.remaining <= 0 then
            ...
            finishJob(jobPath, job, baseFailed or job.standard_previews_failed > 0)
        end
    end
    for _, photo in ipairs(photos) do
        local handle=photo:requestJpegThumbnail(size,size,function(data,errorMessage) completeOne(data ~= nil,errorMessage) end)
    ...
    LrTasks.startAsyncTask(function()
        ...
        if not active or active.finished then return end
        active.finished=true
        job.standard_previews_failed=job.standard_previews_failed+math.max(0,active.remaining or 0)
        job.standard_previews_status='partial'
        job.error=job.error or 'Tempo limite das visualizações padrão excedido.'
...
local function processSource(catalog, job, source, progress, jobPath, importedPhotos, allowed)
    ...
    for _, photoPath in ipairs(files) do
        ...
        if result=='imported' then progress.imported=progress.imported+1; table.insert(photosForCollection,photo); table.insert(importedPhotos,photo)
        elseif result=='skipped' then progress.skipped=progress.skipped+1; table.insert(photosForCollection,photo)
        else progress.failed=progress.failed+1; progress.error=tostring(err) .. ': ' .. tostring(photoPath); appendJobEvent(job,'file_missing','Arquivo ignorado',progress.error,'warning') end
        refreshTotals(job); safeWriteJob(jobPath,job)
        LrTasks.yield()
    end
    ...
    if (job.request or {}).create_collections ~= false and #photosForCollection > 0 then
        ...
        else progress.error='Coleção não criada: ' .. tostring(collectionErr) end
    end
...
local function processJob(jobPath,job)
    ...
    if not catalog then job.status='failed'; job.error='nenhum catálogo ativo'; job.finished_at=timestamp(); safeWriteJob(jobPath,job); processing=false; return false end
    job.active_catalog_path=catalog:getPath(); job.status='running'; job.error=nil; job.started_at=timestamp(); safeWriteJob(jobPath,job)
    ...
    for index,source in ipairs(sources) do
        ...
        if type(progress)~='table' then progress={status='queued',discovered=0,imported=0,skipped=0,failed=0}; job.progress[index]=progress end
        if processSource(catalog,job,source,progress,jobPath,importedPhotos,allowed) then failed=true end
    end
    local presetOk=applyPreset(catalog,importedPhotos,job); safeWriteJob(jobPath,job)
    local smartOk=buildSmartPreviews(catalog,importedPhotos,job); safeWriteJob(jobPath,job)
    startStandardPreviews(importedPhotos,jobPath,job,failed or not presetOk or not smartOk)
    return true
end
...
function Runner.processQueuedOnce()
    ...
    for path in LrFileUtils.files(jobsDir()) do
        if isJobFile(path) then
            ...
            if not job then plainLog('JSON_INVALID path=' .. tostring(path) .. ' error=' .. tostring(readError))
            elseif tostring(job.status)=='queued' then processing=true; writeState('last_scan.txt',timestamp() .. '\ninspected=' .. inspected .. '\nprocessed=1'); processJob(path,job); return 1 end
        end
    end
...
function Runner.runLoop(shouldStop)
    LrFileUtils.createAllDirectories(jobsDir())
    plainLog('Plugin V4.5 iniciado; operações SDK sem pcall; monitorando ' .. jobsDir())
    ...
    processing=false
    plainLog('Plugin V4.5 loop encerrado')
end
